...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) FROM departments").fetchone()[0] == 0:
        ...
        for _cn,_cd in [("general","General"),("announcements","Announcements"),("logistics","Logistics"),("random","Random")]:
            if not conn.execute("SELECT id FROM channels WHERE name=?",(_cn,)).fetchone():
                import uuid as _u2
                conn.execute("INSERT INTO channels(id,name,description,created_by)VALUES(?,?,?,?)",(str(_u2.uuid4()),_cn,_cd,"system"))
        conn.commit()
    conn.close()
...
# ═══════ EMPLOYEE PHOTO + EDIT ═══════
@app.post("/api/employees/{eid}/photo")
def set_photo(eid: str, data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin"], db)
    p = data.get("photo_url","")
    if not p: raise HTTPException(400,"No photo")
    db.execute("UPDATE employees SET photo_url=? WHERE id=?", (p, eid))
    db.commit()
    return {"message":"Photo saved"}

@app.patch("/api/employees/{eid}/edit")
def edit_employee(eid: str, data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin"], db)
    ...
    if upd:
        ...
        db.execute(f"UPDATE employees SET {sets}, updated_at=datetime('now') WHERE id=?",
                   list(upd.values())+[eid])
        if "role" in upd:
            db.execute("UPDATE users SET role=? WHERE id=(SELECT user_id FROM employees WHERE id=?)",(upd["role"],eid))
        if "department" in upd:
            db.execute("UPDATE users SET department=? WHERE id=(SELECT user_id FROM employees WHERE id=?)",(upd["department"],eid))
        db.commit()
    return {"message":"Updated"}

@app.get("/api/employees/directory")
def emp_directory(authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    rows = db.execute("SELECT id,user_id,name,email,phone,department,role,title,photo_url,color FROM employees WHERE status='active' ORDER BY name").fetchall()
    return [dict(r) for r in rows]

# ═══════ MAIL ═══════
@app.get("/api/mail/inbox")
def mail_inbox(authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    import json as _j
    rows = db.execute("SELECT * FROM mail ORDER BY created_at DESC LIMIT 200").fetchall()
    out = []
    for r in rows:
        d = dict(r)
        to_ids = (d.get("to_user_ids") or "").split(",")
        deleted = _j.loads(d.get("deleted_by") or "[]")
        if user["role"]=="admin" or user["id"] in to_ids:
            if user["id"] not in deleted:
                d["unread"] = user["id"] not in _j.loads(d.get("is_read") or "{}")
                out.append(d)
    return out

@app.get("/api/mail/sent")
def mail_sent(authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    rows = db.execute("SELECT * FROM mail WHERE sender_id=? ORDER BY created_at DESC",(user["id"],)).fetchall()
    return [dict(r) for r in rows]

@app.post("/api/mail/send")
def mail_send(data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    import json as _j
    if not data.get("subject"): raise HTTPException(400,"Subject required")
    to_ids = data.get("to_user_ids") or []
    if not to_ids: raise HTTPException(400,"No recipients")
    if isinstance(to_ids, str): to_ids=[to_ids]
    emp = db.execute("SELECT name FROM employees WHERE user_id=?",(user["id"],)).fetchone()
    sname = emp["name"] if emp else user["email"]
    mid = _id()
    db.execute(
        "INSERT INTO mail(id,sender_id,sender_name,sender_email,to_user_ids,to_names,subject,body,attachments,is_read,deleted_by)VALUES(?,?,?,?,?,?,?,?,?,?,?)",
        (mid,user["id"],sname,user["email"],",".join(to_ids),
         data.get("to_names",""),data["subject"],data.get("body",""),
         _j.dumps(data.get("attachments",[])),_j.dumps({}),_j.dumps([]))
    )
    db.commit()
    for rid in to_ids:
        _notify(rid,f"Mail from {sname}",data["subject"][:60],"in")
    return {"message":"Sent","id":mid}

@app.post("/api/mail/{mid}/read")
def mail_mark_read(mid: str, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    import json as _j
    m = db.execute("SELECT is_read FROM mail WHERE id=?",(mid,)).fetchone()
    if not m: raise HTTPException(404)
    reads = _j.loads(m["is_read"] or "{}")
    reads[user["id"]] = _now()
    db.execute("UPDATE mail SET is_read=? WHERE id=?",(_j.dumps(reads),mid))
    db.commit()
    return {"message":"ok"}

@app.delete("/api/mail/{mid}")
def mail_delete(mid: str, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    import json as _j
    m = db.execute("SELECT deleted_by FROM mail WHERE id=?",(mid,)).fetchone()
    if not m: raise HTTPException(404)
    d = _j.loads(m["deleted_by"] or "[]")
    if user["id"] not in d: d.append(user["id"])
    db.execute("UPDATE mail SET deleted_by=? WHERE id=?",(_j.dumps(d),mid))
    db.commit()
...
def create_channel(data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin"], db)
    name = (data.get("name") or "").lower().replace(" ","-").strip("#")
    if not name: raise HTTPException(400,"Name required")
    try:
        db.execute("INSERT INTO channels(id,name,description,created_by,is_private)VALUES(?,?,?,?,?)",
                   (_id(),name,data.get("description",""),user["email"],1 if data.get("is_private") else 0))
        db.commit()
        return {"message":f"#{name} created"}
    except: raise HTTPException(400,"Channel name already exists")

@app.delete("/api/channels/{cid}")
def delete_channel(cid: str, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin"], db)
...
def channel_msgs(cid: str, since: str = None, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    if since:
        rows = db.execute(
            "SELECT * FROM channel_messages WHERE channel_id=? AND created_at>? ORDER BY created_at ASC LIMIT 100",
            (cid,since)
        ).fetchall()
    else:
        rows = db.execute(
...
def post_channel_msg(cid: str, data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    import json as _j
    content = data.get("content","").strip()
    attachments = data.get("attachments",[])
    if not content and not attachments: raise HTTPException(400,"Empty message")
    emp = db.execute("SELECT name,color FROM employees WHERE user_id=?",(user["id"],)).fetchone()
    sname = emp["name"] if emp else user["email"]
    scolor = emp["color"] if emp else "#FF6B00"
    mid = _id()
    db.execute(
        "INSERT INTO channel_messages(id,channel_id,sender_id,sender_name,sender_color,content,attachments)VALUES(?,?,?,?,?,?,?)",
        (mid,cid,user["id"],sname,scolor,content,_j.dumps(attachments))
    )
    db.commit()
    return {"message":"Sent","id":mid}
...
def get_tasks(authorization: str = Header(None), db=Depends(get_db)):
    ...
    elif user["role"] in ["approval_manager","dept_manager"]:
...
def create_task(data: dict, authorization: str = Header(None), db=Depends(get_db)):
    ...
    emp = db.execute("SELECT name FROM employees WHERE user_id=?",(user["id"],)).fetchone()
    by_name = emp["name"] if emp else user["email"]
    tid = _id()
    db.execute(
        ...
        (tid,data["title"],data.get("description",""),
         ...
         user["id"],by_name,data.get("department",""),
...
def update_task(tid: str, data: dict, authorization: str = Header(None), db=Depends(get_db)):
    user = _require_role(authorization, ["admin","manager","approval_manager","dept_manager","employee"], db)
    allowed = ["status","progress_note","priority","due_date"]
    upd = {k:v for k,v in data.items() if k in allowed}
    if upd:
        upd["updated_at"] = _now()
        sets = ", ".join(k+"=?" for k in upd)
        db.execute(f"UPDATE tasks SET {sets} WHERE id=?",list(upd.values())+[tid])
        db.commit()
    return {"message":"Updated"}
